...
# For startup analysis, turn the random universe assignment off
if not startup:
    if not restart_calculation:
...
for det_name in detectors: # Loop through the detectors again and write detector information to the main input file
    s = f'det {det_name} dc {pbed_cell_name} dl {pbed_universe_name} '  # Basic detector information
...
if domain_decomposition:
    if startup:
        ...
        first_pbed.read_dataframe(data.loc[data['r_dist'] <= r_bed])
        ...
        for i,cen in enumerate(bank_centers):
            first_pbed = Pebble_bed()
            first_pbed.read_dataframe(data.loc[(data['x'] > cen - r_bed) & (data['x'] < cen + r_bed)].reset_index(drop = True))
            first_pbed.decompose_in_domains(decomposition_domains, decomposition_types, center = [cen,0,0])
            first_pbed.data['domain_id'].astype(int).to_csv(f'initial_domains{i+2}.txt', index=False, header=False)
            init_domain_files.append(f'initial_domains{i+2}.txt')
            data.loc[data['isactive'] & (data['x'] > cen - r_bed) & (data['x'] < cen + r_bed)  , 'domain_id'] = first_pbed.data.loc[data['isactive'], 'domain_id']
            
        with open('initial_domains.txt', 'wb') as wf:
            for f in init_domain_files:
                with open(f,'rb') as in_f:
                    shutil.copyfileobj(in_f,wf)
...
# Count
if startup:
    num_core = len(data.loc[data['r_dist'] <= r_bed])
    print_with_timestamp(f'{num_core} pebbles in the core:')
    for uni_id, (uni_name, uni) in enumerate(pebbles_dict.items()):
...
for var in ["Nrows_to_move", "neutrons_per_cycle", "power_normalization_value", "DEM_step_increment", "target_fuel_frac"]:
...
pbed.cycle_hist = pd.DataFrame(columns=['time', 'passes', 'recirculated', 'discarded', 'keff', 'keff_relative_uncertainty', 'keff_absolute_uncertainty', 'fuel_fraction']+list(step_wise_variables.keys())) # keff at each time step
...
for step in range(first_step, Nsteps):
    ...
    if startup:
        pbed.cycle_hist.loc[step, 'fuel_fraction'] = pbed.data.loc[pbed.data['r_dist']<= r_bed,'isactive'].sum()/len(pbed.data.loc[pbed.data['r_dist']<= r_bed,'isactive'])
    ...
    #### Other than first step, move pebbles, recirculate, discard, replace with fresh fuel
        ...
        if discrete_motion:
            ...
            if startup:
                #Need to shuffle recirculating separately for core and bank
                for i in range(0, len(zrows[::2])):
                    if motion_direction * zrows[2*i] > motion_direction * zlim:
                        random_state = np.random.randint(1,1000)
                        beyond_lim_core = ((((new_positions['z']-zrows[2*i]).abs()<1e-3) | ((new_positions['z']-zrows[2*i+1]).abs()<1e-3)) & ((new_positions['x']**2+ new_positions['y'] **2)< r_bed**2))
                        new_positions[beyond_lim_core] = new_positions[beyond_lim_core].sample(frac=1, random_state=random_state).values
                        
                        for cen in bank_centers:
                            beyond_lim_outside = ((((new_positions['z']-zrows[2*i]).abs()<1e-3) | ((new_positions['z']-zrows[2*i+1]).abs()<1e-3)) & (new_positions['x'] > cen- r_bed) & (new_positions['x'] < cen + r_bed))
                            new_positions[beyond_lim_outside] = new_positions[beyond_lim_outside].sample(frac=1, random_state=random_state).values
            else:
        ...
        if startup:
            pbed.data['recirculated'] = ((-motion_direction*(pbed.data['z'] - pbed.old_data['z']) > recirc_threshold) & (pbed.old_data['r_dist'] < r_bed)) # select recirculated pebbles (deltaZ > threshold)
        else:
        ...
        if startup:
            ...
            for ind in replace_indices:
                if pbed.cycle_hist.loc[step, 'fuel_fraction'] < target_fuel_frac:
                    ...
                    mat_i = pbed.data.loc[ind, 'mat_name']
                    mat_ind = 0
