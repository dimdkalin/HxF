...
for uni_id, (uni_name, uni) in enumerate(threshold_pebbles_dict.items()):
    ...
    try: # Add only the ones in the input
        ...
        elif isinstance(uni['threshold'],str) and uni['threshold'] == 'top8':
             step_wise_variables[var] = [0] * Nsteps
        else:
            try: # If single value, make list of the same value at every step
                _ = len(step_wise_variables[var])
...
for step in range(first_step, Nsteps):
    ...
    #### Other than first step, move pebbles, recirculate, discard, replace with fresh fuel
        ...
        for uni_id, (uni_name, uni) in enumerate(threshold_pebbles_dict.items()):
            ...
            #Get burnups of all discharged pebbles and sort in reverse order
            discharged_pebs = (pbed.data[f'pebble_type_{uni_id}']) & (pbed.data['recirculated'])
            num_discharged = discharged_pebs.sum()
            burnups = np.zeros(num_discharged)
            i=0
            for index,item in enumerate(discharged_pebs):
                if(item):                
                    burnups[i] = pbed.data.loc[index,'burnup']
                    i+=1
            burnups = np.sort(burnups)
            burnups = burnups[::-1]
            top = int(num_discharged/8)
